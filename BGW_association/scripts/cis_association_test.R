...
# --------------------------------------------------------------------------
# BGW association test (cis only)
#
# Same as association_test.R, but restricts each gene's weights to cis SNPs
# (Trans == 0) before testing. Genes whose weights contain no cis SNPs are
# written to a separate "zero_cis_genes" list and dropped from the results.
#
# Usage:
#   Rscript cis_association_test.R <gene_list_file> <chr> <num_cores>
#
# Data paths are read from config.R. Resolution order:
#   1. environment variable CONFIG_R
...
zero_cis_genes <- assoc_out_dt$GeneName[assoc_out_dt$zero_cis == TRUE]

if (length(zero_cis_genes) > 0) {
  zero_cis_file <- file.path(RESULT_DIR, paste0("CHR_", chr, "_zero_cis_genes.txt"))
  writeLines(zero_cis_genes, con = zero_cis_file)
  assoc_out_dt <- assoc_out_dt[assoc_out_dt$zero_cis == FALSE, ]
}

assoc_out_dt$zero_cis <- NULL

out_file <- file.path(RESULT_DIR, paste0("CHR_", chr, "_cis_assoc.tsv"))
